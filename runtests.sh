...
### Created by John D. Fisher ( jfishe ) on 2025-07-13
...
readonly script_author="jdfenw@gmail.com"
readonly script_created="2025-07-13"
readonly run_as_root=-1 # run_as_root: 0 = don't check anything / 1 = script MUST run as root / -1 = script MAY NOT run as root
readonly script_description="Compare generated ctags file against expected."
...
function Script:main() {
  ...
  case "${action,,}" in
  action1)
    #TIP: use «$script_prefix action1» to run tests.
    #TIP:> $script_prefix action1
    do_action1
    ;;

  action2)
    #TIP: use «$script_prefix action2» to regenerate expected tags files.
...
function do_action1() {
  ...
  ctags_param=(
    --options="$script_install_folder"/ctags.d
    --sort=yes
    --tag-relative=always
  )
  readtags_param=(
  ...
  IO:log "Ctags executable: $(which ctags)" \
    "Ctags parameters: ${ctags_param[*]}"

  for i in "$script_install_folder"/tests/*.md; do
    tagsout="${i%.md}"
    ctags "${ctags_param[@]}" -o "${tagsout}.tags" "$i" 2> "${tagsout}.out"

    tags=$(readtags --tag-file "${tagsout}.tags" "${readtags_param[@]}")
    expected=$(readtags --tag-file "${tagsout}.expectedtags" "${readtags_param[@]}")
    diff <(echo "$expected" ) <(echo "$tags")

    if [ -n "${overwrite+set}" ]; then
      ctags "${ctags_param[@]}" -o "${tagsout}.expectedtags" "$i" 2>> "${tagsout}.out"
    fi
  done
